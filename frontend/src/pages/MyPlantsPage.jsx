...
export function MyPlantsPage({
  ...
  const [propagationForm, setPropagationForm] = useState({ name: "", species: "", startedAt: new Date().toISOString().slice(0, 10), photo: null });
  ...
  const createPropagation = async (event) => {
    event.preventDefault();
    try {
      let plantPhoto = null, propagationEvolution = [];
      if (propagationForm.photo?.file) {
        const { blob } = await preparePropagationPhoto(propagationForm.photo.file);
        plantPhoto = (await userDataApi.uploadPhoto(new File([blob], "esqueje-inicial.jpg", { type: "image/jpeg" }))).url;
        propagationEvolution = [{ id: globalThis.crypto?.randomUUID?.() || Date.now(), date: propagationForm.startedAt, url: plantPhoto, analysis: "Fotografía inicial del esqueje en agua." }];
      }
      await addPlant({ nombreComun: propagationForm.name, nombreCientifico: propagationForm.species || propagationForm.name, categoria: "Esqueje en agua", collectionStatus: "propagation", propagationStartedAt: propagationForm.startedAt, propagationStatus: "enraizando", propagationEvolution, plantPhoto });
      if (propagationForm.photo?.preview) URL.revokeObjectURL(propagationForm.photo.preview);
      setPropagationForm({ name: "", species: "", startedAt: new Date().toISOString().slice(0, 10), photo: null });
      notify("Esqueje añadido a Propagación.");
    } catch (error) { notify(error.message); }
  };
  const selectInitialPropagationPhoto = (event) => {
    const file = event.target.files?.[0]; event.target.value = "";
    if (!file) return;
    if (propagationForm.photo?.preview) URL.revokeObjectURL(propagationForm.photo.preview);
    setPropagationForm({ ...propagationForm, photo: { file, preview: URL.createObjectURL(file) } });
  };
