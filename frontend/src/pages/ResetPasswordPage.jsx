...
export function ResetPasswordPage({ notify }) {
  ...
  return <section className="auth-page"><div className="auth-art"><span><KeyRound size={30} /></span><h1>Vuelve a<br />tu jardín.</h1><p>Crea una contraseña nueva y recupera tus plantas, diagnósticos y recordatorios.</p></div><div className="auth-panel"><div className="auth-card"><span className="kicker">RECUPERAR ACCESO</span><h2>Nueva contraseña</h2><p>Utiliza al menos ocho caracteres; el contenido lo eliges tú.</p><form onSubmit={submit}><label>Nueva contraseña<input required minLength="8" type="password" value={password} onChange={(event) => setPassword(event.target.value)} /></label><label>Repetir contraseña<input required minLength="8" type="password" value={repeat} onChange={(event) => setRepeat(event.target.value)} /></label><button className="primary auth-submit" disabled={loading}>{loading ? <span className="spinner" /> : <KeyRound size={18} />} {loading ? "Actualizando…" : "Guardar contraseña"}</button></form><Link className="auth-switch" to="/acceso">Volver al inicio de sesión</Link></div></div></section>;
}
