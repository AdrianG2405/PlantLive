...
import { getSavedPassword, hasNativePasswordManager, savePassword } from "../utils/passwordManager";
...
export function AuthPage({ notify }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (mode === "login") savePassword(form.email.trim(), form.password).catch(() => {});
  ...
  return <section className="auth-page"><div className="auth-art"><span><Leaf size={30} /></span><h1>Tu jardín,<br />siempre contigo.</h1><p>Guarda plantas, diagnósticos y recordatorios en una cuenta personal.</p></div>
    <div className="auth-panel"><div className="auth-card"><span className="kicker">{mode === "login" ? "BIENVENIDO" : mode === "forgot" ? "RECUPERAR ACCESO" : "EMPIEZA TU JARDÍN"}</span><h2>{mode === "login" ? "Iniciar sesión" : mode === "forgot" ? "¿Olvidaste la contraseña?" : "Crear una cuenta"}</h2>
      ...
      {mode === "login" && hasNativePasswordManager() && <button type="button" className="auth-switch" onClick={async () => { try { const saved = await getSavedPassword(); if (saved) setForm((current) => ({ ...current, email: saved.email, password: saved.password })); } catch { notify("No se eligió ninguna cuenta guardada."); } }}><KeyRound size={17} /> Usar cuenta guardada</button>}
      <form onSubmit={submit} noValidate autoComplete="on">{mode === "register" && <label className={errors.name ? "field-invalid" : ""}>Nombre<input required name="name" autoComplete="name" minLength="2" value={form.name} onChange={(event) => { setForm({ ...form, name: event.target.value }); setErrors({ ...errors, name: "" }); }} placeholder="Tu nombre" aria-invalid={Boolean(errors.name)} />{errors.name && <small className="field-error">{errors.name}</small>}</label>}
        <label className={errors.email ? "field-invalid" : ""}>Email<input required type="email" name="email" inputMode="email" autoComplete={mode === "login" ? "username" : "email"} autoCapitalize="none" spellCheck="false" value={form.email} onChange={(event) => { setForm({ ...form, email: event.target.value }); setErrors({ ...errors, email: "" }); }} placeholder="nombre@gmail.com" aria-invalid={Boolean(errors.email)} />{mode === "register" && !errors.email && <small className="field-help">Debe terminar en .com para reducir errores antes de enviar la verificación.</small>}{errors.email && <small className="field-error">{errors.email}</small>}</label>
        {mode !== "forgot" && <label className={errors.password ? "field-invalid" : ""}>Contraseña<div className="password-field"><input required minLength="8" name="password" autoComplete={mode === "register" ? "new-password" : "current-password"} type={showPassword ? "text" : "password"} value={form.password} onChange={(event) => { setForm({ ...form, password: event.target.value }); setErrors({ ...errors, password: "" }); }} placeholder={mode === "register" ? "Elige tu contraseña" : "Tu contraseña"} aria-invalid={Boolean(errors.password)} /><button type="button" onClick={() => setShowPassword(!showPassword)}>{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div>{mode === "register" && !errors.password && <small className="field-help">Como mínimo 8 caracteres; el contenido lo eliges tú.</small>}{errors.password && <small className="field-error">{errors.password}</small>}</label>}
